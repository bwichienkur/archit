...
function project(p:Point2,a:Point2,b:Point2){const dx=b.x-a.x,dy=b.y-a.y,length2=dx*dx+dy*dy;if(!length2)return{point:a,t:0,distance:Math.hypot(p.x-a.x,p.y-a.y)};const t=((p.x-a.x)*dx+(p.y-a.y)*dy)/length2;const point={x:a.x+dx*t,y:a.y+dy*t};return{point,t,distance:Math.hypot(p.x-point.x,p.y-point.y)};}
function rotatePoint(p:Point2,pivot:Point2,angle:number){const x=p.x-pivot.x,y=p.y-pivot.y,c=Math.cos(angle),s=Math.sin(angle);return{x:pivot.x+x*c-y*s,y:pivot.y+x*s+y*c};}
function mirrorPoint(p:Point2,a:Point2,b:Point2){const projection=project(p,a,b).point;return{x:2*projection.x-p.x,y:2*projection.y-p.y};}
function midpoint(a:Point2,b:Point2){return{x:(a.x+b.x)/2,y:(a.y+b.y)/2};}
