...
export class HttpCatalogGateway {
  ...
  async upsert(product:Product){const response=await fetch(`${this.baseUrl}/api/catalog/products`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({externalId:product.id,manufacturer:product.manufacturer,sku:product.sku,name:product.name,category:product.category,unitOfMeasure:product.unitOfMeasure,payload:product})});return readJson<CatalogProductRecord>(response);}
  async get(id:string){return readJson<CatalogProductRecord>(await fetch(`${this.baseUrl}/api/catalog/products/${encodeURIComponent(id)}`));}
  async search(filters:{manufacturer?:string;category?:string;q?:string}={}){const query=new URLSearchParams();if(filters.manufacturer)query.set('manufacturer',filters.manufacturer);if(filters.category)query.set('category',filters.category);if(filters.q)query.set('q',filters.q);return readJson<CatalogProductRecord[]>(await fetch(`${this.baseUrl}/api/catalog/products?${query}`));}
}
