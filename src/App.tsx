...
function BuildingModel3D({ model, selectedWallId, onSelectWall }: { model: BuildingModelV2; selectedWallId: string | null; onSelectWall: (id: string) => void }) {
  ...
  return <Canvas camera={{ position: [8, 8, 9], fov: 45 }}><ambientLight intensity={1.5}/><directionalLight position={[5,10,5]} intensity={2}/><Grid args={[20,20]} cellSize={.5} sectionSize={2}/>{model.walls.map(wall => {
    const dx = wall.end.x - wall.start.x;
    const dz = wall.end.y - wall.start.y;
    const length = Math.hypot(dx, dz) * geometry.scale;
    const mx = ((wall.start.x + wall.end.x) / 2 - geometry.centerX) * geometry.scale;
    const mz = ((wall.start.y + wall.end.y) / 2 - geometry.centerY) * geometry.scale;
    const height = Math.max(wall.height * geometry.scale, .05);
    const thickness = Math.max(wall.thickness * geometry.scale, .03);
    const angle = -Math.atan2(dz, dx);
    return <mesh key={wall.id} position={[mx, height / 2, mz]} rotation={[0, angle, 0]} onClick={event=>{event.stopPropagation();onSelectWall(wall.id)}}><boxGeometry args={[length,height,thickness]}/><meshStandardMaterial color={selectedWallId===wall.id ? '#d9a441' : wall.lineage.validationState==='modified' ? '#d8bd83' : '#d7dadd'}/></mesh>;
...
function App() {
  ...
  const { model: buildingModel, selection: buildingSelection, select: selectBuilding, updateWall: updateBuildingWall, clear: clearBuilding } = useBuildingEditorStore();
