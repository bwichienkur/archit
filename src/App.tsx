...
function App() {
  ...
  const { model: buildingModel, selection: buildingSelection, select: selectBuilding, updateWall: updateBuildingWall, updateOpening: updateBuildingOpening, undo: buildingUndo, redo: buildingRedo, canUndo: buildingCanUndo, canRedo: buildingCanRedo, error: buildingError, clear: clearBuilding } = useBuildingEditorStore();
...
function BuildingOpeningInspector({ opening, units, onUpdate }: { opening: BuildingModelV2['openings'][number]; units: BuildingModelV2['geometryUnits']; onUpdate: (patch: Partial<Pick<BuildingModelV2['openings'][number], 'offsetFromWallStart'|'width'|'height'|'sillHeight'>>) => void }) {
  return <><div className={`validation ${opening.lineage.validationState}`}>Hosted BIM {opening.kind} • {opening.lineage.validationState}</div><section><h3>OPENING</h3><Field label="Host wall" value={opening.hostWallId}/><NumericField label={`Width (${units})`} value={opening.width} onChange={value=>onUpdate({width:value})}/><NumericField label={`Height (${units})`} value={opening.height} onChange={value=>onUpdate({height:value})}/><NumericField label={`Offset (${units})`} value={opening.offsetFromWallStart} min={0} onChange={value=>onUpdate({offsetFromWallStart:value})}/>{opening.kind==='window' && <NumericField label={`Sill (${units})`} value={opening.sillHeight ?? 0} min={0} onChange={value=>onUpdate({sillHeight:value})}/>}<Field label="Subtype" value={opening.subtype ?? '—'}/></section><section><h3>CAD LINEAGE</h3><p className="muted">{opening.lineage.sourceCadEntityIds.length} immutable source entities • {Math.round((opening.lineage.confidence ?? 1)*100)}% inference confidence</p>{opening.lineage.sourceCadEntityIds.map(id=><div className="cad-id" key={id}>{id}</div>)}</section></>;
}
...
function NumericField({label,value,onChange,min=Number.MIN_VALUE}:{label:string,value:number,onChange:(value:number)=>void;min?:number}) { return <div className="field"><label>{label}</label><input value={value} type="number" min={min} step="0.25" onChange={e=>{const next=Number(e.target.value); if(Number.isFinite(next)&&next>=min) onChange(next)}}/></div> }
export default App;
