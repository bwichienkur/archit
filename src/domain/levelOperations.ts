import type { ArchitecturalRoom, ArchitecturalWall, BuildingModelV2, Level, Point2, WallOpening } from './building';
...
export function copyLevelGeometry(model:BuildingModelV2,sourceLevelId:string,targetLevelId:string,offset:Point2={x:0,y:0}):BuildingModelV2 {
  ...
  const openings:WallOpening[]=model.openings.filter(opening=>idMap.has(opening.hostWallId)).map(opening=>({...opening,id:`${opening.id}:copy:${targetLevelId}`,hostWallId:idMap.get(opening.hostWallId)!,lineage:{...opening.lineage,validationState:'modified' as const}}));
