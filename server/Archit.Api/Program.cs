...
app.MapPost("/api/cad/imports", async (HttpRequest request, ICadImportProvider provider, ICadArtifactStore artifacts, ICadImportQueue queue, ICadImportJobStore jobs, CancellationToken cancellationToken) =>
{
    ...
    if (file.Length > maxFileBytes) return Results.BadRequest(new { error = "DWG exceeds the 250 MB import limit." });
...
app.MapPost("/api/projects", async (CreateProjectRequest request, IProjectRepository repository, CancellationToken cancellationToken) =>
{
    ...
    var project = await repository.CreateAsync(request.Name, request.TenantId, cancellationToken);
    return Results.Created($"/api/projects/{project.Id}", project);
});

app.MapGet("/api/projects", async (IProjectRepository repository, CancellationToken cancellationToken) => Results.Ok(await repository.ListAsync(cancellationToken)));
app.MapGet("/api/projects/{projectId:guid}", async (Guid projectId, IProjectRepository repository, CancellationToken cancellationToken) => { var project = await repository.GetAsync(projectId, cancellationToken); return project is null ? Results.NotFound() : Results.Ok(project); });
...
app.MapPost("/api/projects/{projectId:guid}/revisions", async (Guid projectId, CreateRevisionRequest request, IProjectRepository repository, CancellationToken cancellationToken) =>
{
    ...
    try { var revision = await repository.AddRevisionAsync(projectId, request, cancellationToken); return Results.Created($"/api/projects/{projectId}/revisions/{revision.Id}", revision); }
    catch (KeyNotFoundException) { return Results.NotFound(); }
    catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); }
});

app.MapGet("/api/projects/{projectId:guid}/revisions", async (Guid projectId, IProjectRepository repository, CancellationToken cancellationToken) => { try { return Results.Ok(await repository.ListRevisionsAsync(projectId, cancellationToken)); } catch (KeyNotFoundException) { return Results.NotFound(); } });
...
app.MapGet("/api/tenants/{tenantId:guid}", async (Guid tenantId, ITenantRepository repository, CancellationToken cancellationToken) => { var tenant = await repository.GetAsync(tenantId, cancellationToken); return tenant is null ? Results.NotFound() : Results.Ok(tenant); });
app.MapPut("/api/tenants/{tenantId:guid}/memberships", async (Guid tenantId, UpsertMembershipRequest request, ITenantRepository repository, CancellationToken cancellationToken) => { try { return Results.Ok(await repository.UpsertMembershipAsync(tenantId, request, cancellationToken)); } catch (KeyNotFoundException) { return Results.NotFound(); } catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); } });
...
{
    if (!string.Equals(request.Format,"json",StringComparison.OrdinalIgnoreCase)) return Results.BadRequest(new { error = $"Server-side export format '{request.Format}' is not configured. JSON is currently available; DWG/DXF/IFC require format-specific providers." });
    try { var job = await repository.CreateAsync(projectId, request, cancellationToken); return Results.Accepted($"/api/exports/{job.Id}", job); } catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); }
});
app.MapGet("/api/projects/{projectId:guid}/exports", async (Guid projectId, IExportJobRepository repository, CancellationToken cancellationToken) => Results.Ok(await repository.ListAsync(projectId, cancellationToken)));
